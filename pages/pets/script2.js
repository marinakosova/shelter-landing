...
var windowWidth = updateWindowSize();
var slidesToShow = GetNumberOfSlidesToShow(windowWidth);
var data = [];
...
function initSlider(data, slidesToShow) {
    ...
    if (slidesToShow >= 6 && data.length >= 6) {
        track.appendChild(createOneSlider(data[3]));
        track.appendChild(createOneSlider(data[4]));
        track.appendChild(createOneSlider(data[5]));
    }
    if (slidesToShow == 8 && data.length >= 8) {
        track.appendChild(createOneSlider(data[6]));
        track.appendChild(createOneSlider(data[7]));
    }
}

function GetNumberOfSlidesToShow(windowWidth) {
    slidesToShow = 3;
...
window.onresize = function onWindowResize() {
    var windowWidth = updateWindowSize();
    slidesToShow = GetNumberOfSlidesToShow(windowWidth);
    initSlider(data, slidesToShow);
}
...
window.onload = async function() {
    ...
    //slider
    let windowWidth = updateWindowSize();
    let slidesToShow = GetNumberOfSlidesToShow(windowWidth);
    initSlider(data, slidesToShow);
